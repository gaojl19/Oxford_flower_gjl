...
def ConvMixer_768_32_clr(num_classes: int = 1000, **kwargs):
    model = ConvMixer_clr(dim = 768, depth = 32, 
...
def MLPMixer_S_16_clr(num_classes: int = 1000, **kwargs):

    model = MLPMixer_clr(patch_size = 16, d_model = 512, depth = 12, 
...
# TRANSFORMER
class VisionTransformer_clr(nn.Module):
    ...
    def __init__(self,
        ...
        super(VisionTransformer_clr,self).__init__()
...
def vit_small_patch16_224_clr(pretrained=False, **kwargs):
    if pretrained:
        # NOTE my scale was wrong for original weights, leaving this here until I have better ones for this model
        kwargs.setdefault('qk_scale', 768 ** -0.5)
    
    model = VisionTransformer_clr(patch_size=16, embed_dim=768, depth=8, num_heads=8, mlp_ratio=3., **kwargs)
    # model = VisionTransformer_clr(patch_size=16, embed_dim=768, depth=8, num_heads=8, mlp_ratio=3., **kwargs)
