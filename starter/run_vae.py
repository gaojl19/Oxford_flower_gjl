...
def build_model(model_type):
    if model_type == "ConvMixer":
        model = ConvMixer_768_32_vae()
    elif model_type == "MLPMixer":
        model = MLPMixer_S_16_vae()
    elif model_type == "VIT":
        model = vit_small_patch16_224_vae()
    else:
...
def train(args, dataloader):
    ...
    for epoch in range(epochs):
        train_loss = train_one_epoch(model, dataloader.traindataset, criterion, optimizer, epoch, 1, scheduler, vae=True)
        acc = valid_one_epoch(model, dataloader.validdataset, epoch, vae=True, criterion=criterion)
        if acc > best_acc:
            best_acc = acc
            best_epoch = epoch
